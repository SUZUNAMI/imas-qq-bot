...
- **更新流程约定（2026-08-28 用户要求）**：用户每次说「更新」时，必须**先做全量对比**再行动：
  ① 本地工作区 vs GitHub 远程（git status / HEAD vs origin/main，含未推送 commit）；
  ② 子任务目录（`子任务-曲库查询/`）vs 主仓库（hash 对比，找未同步的最新修改）；
  ③ 主仓库未提交/未推送状态确认。对比通过后才允许打包/推送/迁移。
